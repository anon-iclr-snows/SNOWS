...
class MLP(torch.nn.Sequential):
    def __init__(
        ...
        for hidden_dim in hidden_channels[:-1]:
            layers.append(MaskedLinear(in_dim, hidden_dim, bias=bias))
        ...
        layers.append(MaskedLinear(in_dim, hidden_channels[-1], bias=bias))
        layers.append(torch.nn.Dropout(dropout, **params))
...
def multi_head_attention_forward(
    ...
    assert in_proj_mask is not None, "in_proj_mask must be provided"
    embed_dim = in_proj_weight.shape[0]
    mask_q, mask_k, mask_v = torch.split(in_proj_mask, embed_dim // 3, dim=0)
    
    def mask_backward_hook(grad):
        # Ensure the gradient for the mask is zero where the mask itself is zero
        return grad * in_proj_mask

    # Register the backward hook on the mask
    in_proj_mask.register_hook(mask_backward_hook)
    ...
    if not use_separate_proj_weight:
        ...
        q_weight = in_proj_weight[:embed_dim, :] * mask_q
        k_weight = in_proj_weight[embed_dim:2*embed_dim, :] * mask_k
        v_weight = in_proj_weight[2*embed_dim:, :] * mask_v
        ...
        q = F.linear(query, q_proj_weight * mask_q, in_proj_bias[:embed_dim])
        k = F.linear(key, k_proj_weight * mask_k, in_proj_bias[embed_dim:2*embed_dim])
        v = F.linear(value, v_proj_weight * mask_v, in_proj_bias[2*embed_dim:])
...
class InProjModule(nn.Module):
    def __init__(self, embed_dim, device=None, dtype=None):
        ...
        self.in_proj_mask = Parameter(torch.ones(self.in_proj_weight.shape, device=device, dtype=dtype))
    ...
    def forward(self, x):
        return nn.functional.linear(x, self.in_proj_weight*self.in_proj_mask, self.in_proj_bias)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output, attn_output_weights = multi_head_attention_forward(
            ...
            self.in_proj_weight, self.in_proj_mask, self.in_proj_bias,
